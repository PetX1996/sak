...
registerTimeLimitDvar( dvarString, defaultValue, minValue, maxValue )
{
	...
	//if ( getDvar( dvarString ) == "" )
		setDvar( dvarString, level.petx_time_limit );
	...
	level.timeLimitDvar = dvarString;	
	level.timelimitMin = 0;
	level.timelimitMax = 1000;
	level.timelimit = level.petx_time_limit;
...
updateGameTypeDvars()
{
	...
	while ( game["state"] == "playing" )
	{
		...
		timeLimit = getValueInRange( level.timeLimit, level.timeLimitMin, level.timeLimitMax );
...
Callback_StartGameType()
{
	...
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		level.prematchPeriod = 0; //maps\mp\gametypes\_tweakables::getTweakableValue( "game", "playerwaittime" );
		level.prematchPeriodEnd = 0; //maps\mp\gametypes\_tweakables::getTweakableValue( "game", "matchstarttime" );
	}
...
{
	level.friendlyfire = 1; //zapnutie teamkill

	if(isdefined(sMeansOfDeath)&&isdefined(iDamage)&&sMeansOfDeath == "MOD_PISTOL_BULLET")
	{
		for(i = 1;i < 7;i++)
		{
			if(sWeapon == level.sak_class["class"+i+"_lovec"]["weapon"] || sWeapon == level.sak_class["class"+i+"_jumper"]["weapon"])
			{
				//iprintln("damage off");
				return;
			}
	...
	if (isdefined(eAttacker) && isdefined(self) && isplayer(self) && isplayer(eAttacker) && isdefined(self.pers["team"]) && isdefined(eAttacker.pers["team"]) 
	&& eAttacker.pers["team"] == self.pers["team"] && self.pers["team"] == "axis" && !level.tk_axis)
		return;
		
	if (isdefined(eAttacker) && isdefined(self) && isplayer(self) && isplayer(eAttacker) && isdefined(self.pers["team"]) && isdefined(eAttacker.pers["team"])
	&& eAttacker.pers["team"] == self.pers["team"]&& self.pers["team"] == "allies" && !level.tk_allies)
		return;		
	...
	{
		if(level.petx_jump_dmg == 0)
			return;
		else if(level.petx_jump_dmg == 2)	
		{
...
//			}
		...
		if ( level.teamBased && isPlayer( eAttacker ) && (self != eAttacker) && (self.pers["team"] == eAttacker.pers["team"]) )
		{
			...
			else if ( level.friendlyfire == 1 ) // the friendly takes damage
			{
				...
				if(isdefined(self)&&isplayer(self)&&isalive(self)&&self.class_special == "shield")
				{
					if(isdefined(eAttacker)&&isplayer(eAttacker)&&isalive(eAttacker)&&eAttacker.pers["team"] == self.pers["team"]&&!self.class_shield&&randomint(3) != 0)
					{
						self.class_shield = true;
						self iprintlnbold("^4Ochranny stit aktivovany!");
						return;
					}
				}
		...
		else
		{
			...
			*/
			...
			/*
			//Damage Knifu podla classu
			*/
			
			if (isdefined(eAttacker)&&isdefined(iDamage)) //modify damage...
			{
				iDamage = self petx\_class::OnPlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime);
			}

			if(isdefined(iDamage)&&!iDamage)
				return;
	...
	if (isplayer(self)&&isplayer(eAttacker)&&isAlive(eAttacker)&&isAlive(self)&&eAttacker.pers["team"] != self.pers["team"] && isdefined(iDamage))
	{
		value = iDamage;
		
		if(!isdefined(game["special_round"]))
			value = iDamage/level.petx_point_damage;
		else
			value = (iDamage/level.petx_point_damage)/2;
		
		value = int(value);
		
		eAttacker thread petx\_upoint::giveupoint(undefined,value);
		
		score = eAttacker.pers["score"];
		maps\mp\gametypes\_globallogic::_setPlayerScore( eAttacker, score+value );
		eAttacker thread maps\mp\gametypes\_rank::giveRankXP( "life", value );
	
		self iprintln("^3Dostal si hit o hodnote ^1"+iDamage+"^3 od hraca ^1"+eAttacker.name);
		eAttacker iprintln("^3Dal si hit o hodnote ^1"+iDamage+"^3 hracovi ^1"+self.name);
		//self notify("health_change");
	}	

	self notify("health_change"); //////////////////////////////////uprava health hudu	
...
//	self maps\mp\gametypes\_weapons::updateWeaponUsageStats();
		...
		else
		{
			...
			else
			{
				...
				if ( sMeansOfDeath == "MOD_HEAD_SHOT" )
				{
					...
					if(!isdefined(game["special_round"]))
						value = level.petx_point_headshot;
					else
						value = int(level.petx_point_headshot/2);
					
					attacker thread petx\_upoint::GiveuPoint( undefined, value );  /////////////////////////////////////////////////////////////////////////////////pridanie  Upoint ////////////////////////////////////////////////
				...
				{
					if(!isdefined(game["special_round"]))
						value = level.petx_point_kill; // 200 bodou za kill
					else
						value = int(level.petx_point_kill/2);
				...
				if(isalive(attacker))
					attacker notify("Killed" , eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration);
	...
	wait ( 0.25 );
	self.cancelKillcam = true; //origo false
